...
export default function Watchlist(props){
    ...
    let [genreList,setGenreist] = useState(["All Genres"]);
    ...
    return(
        <>
            <div className="flex justify-center flex-wrap">
                {genreList.map((genre)=>{
                    return(
                        <div onClick={()=>handleFilter(genre)} key={genre} className={currGenre === genre? "m-4 h-10 w-60 bg-blue-400 rounded-xl text-white flex justify-center items-center font-bold":
            ...
            <div className="flex justify-center my-4">
                <input onChange= {handleSearch} className="h-[3rem] w-[18rem] bg-gray-200 
            ...
            <div className="m-7 overflow-hidden rounded-lg border shadow-md">
                ...
                <tbody>
                    ...
                    .map((movieObj)=>{
                        ...
                        <td className="flex items-center mx-4 py-4"> <img className="h-[6rem] w-[10rem]" src={"https://image.tmdb.org/t/p/original/"+movieObj.poster_path} alt="" />
                            <div className="m-4">{movieObj.title}</div>
                        </td>
                        <td>{movieObj.vote_average}</td>
                        <td>{movieObj.popularity}</td>
                        <td>{genreids[movieObj.genre_ids[0]]}</td>
                        <td onClick = {()=>handleRemoveFromWatchList(movieObj)} className="text-red-500 hover:cursor-pointer"><i className="fa-solid fa-trash"></i></td>
                        </tr>
